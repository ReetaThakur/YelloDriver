...
public class ExtendPassViewModel extends ViewModel {
    //this is the data that we will fetch asynchronously 
    public MutableLiveData<ModelBoardingPassAmount.Data.YtBoardingPassByPk> mutableLiveDataPassAmount = new MutableLiveData<>();

    public void getPassAmount(Query query) {
            ...
            public void onResponse(com.apollographql.apollo.api.@NotNull Response response) {
                if (!response.hasErrors()) {
                    ...
                    if (response.getData() != null) {
                        ...
                        Gson gson = new Gson();
                        ModelBoardingPassAmount modelBoardingPassAmount = gson.fromJson(json, ModelBoardingPassAmount.class);

                        ModelBoardingPassAmount.Data.YtBoardingPassByPk ytBoardingPassByPk = modelBoardingPassAmount.getData().getYtBoardingPassByPk();

                        mutableLiveDataPassAmount.postValue(ytBoardingPassByPk);
                    } else {
                        mutableLiveDataPassAmount.postValue(null);
                    }
                } else {
